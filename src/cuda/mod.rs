...
pub(crate) fn apply_selective_scan(
    u: &Tensor,
    delta: &Tensor,
    a: &Tensor,
    b: &Tensor,
...
pub(crate) fn apply_causal_conv1d(
    x: &Tensor,
    weight: &Tensor,
    bias: Option<&Tensor>,
    ...
    if let Some(seq_idx) = &seq_idx {
        assert!(
            is_channel_last,
            "seq_idx only supported for channel last layout"
        );
    ...
    let ws = weight.stride();
    let (weight_c_stride, weight_width_stride) = (ws[0] as u32, ws[1] as u32);

    let outs = out.stride();
    let (out_batch_stride, out_c_stride, out_l_stride) =
        (outs[0] as u32, outs[1] as u32, outs[2] as u32);

    let (conv_state_batch_stride, conv_state_c_stride, conv_state_l_stride) = (0, 0, 0);
    unsafe {
        let conv_state_ptr = std::ptr::null();
        let x_ptr = x.device_ptr()?;
...
pub(crate) fn apply_causal_conv1d_update(
    ...
    let xs = x.stride();
    let (x_batch_stride, x_c_stride, x_l_stride) = (xs[0] as u32, xs[1] as u32, 0);

    let ws = weight.stride();
    let (weight_c_stride, weight_width_stride) = (ws[0] as u32, ws[1] as u32);

    let outs = out.stride();
    let (out_batch_stride, out_c_stride, out_l_stride) = (outs[0] as u32, outs[1] as u32, 0);

    let conv_states = conv_state.stride();
    let (conv_state_batch_stride, conv_state_c_stride, conv_state_l_stride) = (
        conv_states[0] as u32,
        conv_states[1] as u32,
        conv_states[2] as u32,
    );
    ...
    unsafe {
        ...
        ffi::causal_conv1d_update_ffi(&params, input_dtype, weight_dtype, stream);
    }
    Ok((out, conv_state.clone()))
}
